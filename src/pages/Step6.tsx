...
const Step6 = () => {
  ...
  const splitParagraphIntoSentences = (paragraphText: string, paragraphIndex: number) => {
    // Improved regex to handle abbreviations and ellipses better, but still basic
    const sentences = paragraphText
      .split(/(?<!Mr|Mrs|Ms|Dr)\.(?!\d)|[!?]\s+/) // Split by . ! ? unless preceded by Mr/Mrs/Ms/Dr or followed by a digit (for decimals/versions)
  ...
  const handleSentenceDelete = () => {
    ...
    // Re-join sentences, potentially with a space. Handle empty paragraph case.
  ...
  const handleSentenceMove = (direction: 'up' | 'down') => {
    ...
    // Log this as an edit for tracking progress, although the original/new sentence tracking isn't perfect for moves
    const newHistoryEntry = {
      paragraphIndex: activeParagraphIndex,
      originalSentence: `Sentence moved ${direction} (Original: "${selectedSentence}")`, // Log original for context
      newSentence: newSentences[newIndex], // Log the sentence that ended up in the new position
      timestamp: Date.now()
    };
  ...
  const handleAISuggestions = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json(); // Attempt to parse error response from serverless function
  ...
  const handleSave = (data: EssayData) => {
    if (data) {
      // Ensure step5 data reflects current paragraphs
      if (!data.step5) {
        data.step5 = { paragraphs: [] };
      }
      // Need to save the paragraphs array from Step 6 state,
      // as it might have changed due to sentence edits/deletions/moves
      data.step5.paragraphs = [...paragraphs];

      // Save Step 6 specific data (edit history)
      data.step6 = {
        editHistory: editHistory
      };

      saveEssayData(data);
      toast.success("Step 6 data saved", {
        description: "Your sentence edits and history have been saved.",
      });
    }
  };

  const totalParagraphs = paragraphs.length;
  // Calculate progress based on paragraphs that have *any* edit history entry
  ...
  // Check if the user has made at least 3 edits in total
  ...
  return (
    <StepLayout
      step={6}
      totalSteps={9}
      onSave={() => essayData && handleSave(essayData)} // Pass current essayData to handleSave
    ...
    >
      ...
      {paragraphs.length === 0 ? (
        ...
        <Tabs
          defaultValue={`paragraph-${activeParagraphIndex}`} // Set default value based on state
          className="w-full"
          onValueChange={(value) => {
        ...
        >
          ...
          {/* Use activeParagraphIndex to render only the content for the active tab */}
                        ...
                        <div className="bg-white p-4 rounded-md border border-blue-100 shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <Button
                                    ...
                                    disabled={isLoadingSuggestions} // Disable button while loading
                                    ...
                                    .map((edit, index) => (
                                        <div key={index} className="p-3">
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                {edit.originalSentence !== edit.newSentence && ( // Only show 'Original' if it changed
